...
from tabssl.data import load_sklearn_breast_cancer, make_dataloaders, load_tabular_csv
...
def parse_args():
    p = argparse.ArgumentParser(description="Tabular SSL: Contrastive + Masked Reconstruction (CSV-ready, verbose)")
    ...
    # Data flags
    p.add_argument("--data-kind", type=str, choices=["sklearn_bc","csv"], default="sklearn_bc",
                   help="sklearn_bc = Breast Cancer; csv = custom CSV")
    p.add_argument("--csv-path", type=str, default="", help="Path to CSV if --data-kind csv")
    p.add_argument("--target-col", type=str, default="", help="Target column name in CSV")
    p.add_argument("--test-size", type=float, default=0.2)
    p.add_argument("--categorical-cols", type=str, default="", help="Comma-separated categorical columns")
    p.add_argument("--auto-categorical", action="store_true", help="Infer categorical columns (non-numeric)")
    return p.parse_args()
...
def encode_labels_if_needed(y, outdir):
    # If labels are non-numeric, factorize to integers and save mapping
    if np.issubdtype(y.dtype, np.number):
        return y  # already numeric
    classes, y_int = np.unique(y, return_inverse=True)
    mapping = {int(i): str(c) for i, c in enumerate(classes)}
    os.makedirs(outdir, exist_ok=True)
    with open(os.path.join(outdir, "label_mapping.json"), "w", encoding="utf-8") as f:
        json.dump({"int_to_label": mapping}, f, indent=2)
    print(f"[INFO] Non-numeric target encoded. Mapping saved to outputs/label_mapping.json")
...
def main():
    ...
    # ----- Load data
    if args.data_kind == "csv":
        if not args.csv_path or not args.target_col:
            raise SystemExit("--data-kind csv requires --csv-path and --target-col")
        cat_cols = [s.strip() for s in args.categorical_cols.split(",") if s.strip()] if args.categorical_cols else None
        train, test = load_tabular_csv(
            args.csv_path, args.target_col,
            test_size=args.test_size, seed=args.seed,
            categorical_cols=cat_cols, auto_categorical=args.auto_categorical
        )
        print(f"[INFO] Loaded CSV dataset: {args.csv_path}")
        print(f"[INFO] Train shape={train.X.shape}, Test shape={test.X.shape}, Features={train.X.shape[1]}")
    else:
        train, test = load_sklearn_breast_cancer()
        print("[INFO] Loaded built-in Breast Cancer dataset")
        print(f"[INFO] Train shape={train.X.shape}, Test shape={test.X.shape}, Features={train.X.shape[1]}")

    # Ensure labels are integer-coded (works for binary & multiclass)
    train.y = encode_labels_if_needed(np.asarray(train.y), args.save_dir)
    test.y  = encode_labels_if_needed(np.asarray(test.y),  args.save_dir)

    # ----- Dataloaders & model
    unlabeled_loader, _, _ = make_dataloaders(train, test, batch_size=args.batch_size)
